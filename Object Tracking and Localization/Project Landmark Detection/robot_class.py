...
class robot:
    ...
    def sense(self):
        ...
        for landmark in self.landmarks:
            dx = landmark[0] + -self.x + self.rand()*self.measurement_noise  # check if landmark - self or other way around
            dy = landmark[1] + -self.y + self.rand()*self.measurement_noise  #   doesn't seem to matter
            if (abs(dx) < self.measurement_range) and (abs(dy) < self.measurement_range): 
                measurements.append([idx, dx,dy])
            idx += 1                                 # check if this should if in the if block
        return measurements
